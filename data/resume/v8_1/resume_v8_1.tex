% !TEX root = /home/shantanu/Documents/Shanto/jobs/resume/resume_v8_0.tex`'
...
\resumeItemListStart
\item[$\bullet$] Leading full-stack software development of a \textbf{7-DoF robotic arm} atop a \textbf{mobile robot}, targeting \textbf{Jetson Thor} deployment.
\item[$\bullet$] Ported \textbf{Moteus X1 servo controller} firmware to drive custom \textbf{servo-hydraulic actuators}, extending it beyond stock control.
\item[$\bullet$] Built \textbf{RAII}-based \textbf{CUDA} \textbf{unified memory} \textbf{API} for \textbf{zero-copy CPU-GPU} access on \textbf{DGX}, \textbf{Jetson}.
\item[$\bullet$] Designed a \textbf{collision-aware inverse kinematics (IK)} solver with custom \textbf{CUDA kernels}, converging in \textbf{p95:80 ms}.
...
\item[$\bullet$] Built a \textbf{PCD-to-ESDF} module using \textbf{nvblox} for \textbf{environment-aware motion planning} from 3D sensor data.
\item[$\bullet$] Integrated \textbf{OMPL}'s \textbf{RRT-Connect} with custom \textbf{CUDA kernels} for environmental collision checks.
\item[$\bullet$] Developed a \textbf{PIV cascaded controller} for \textbf{low-level joint actuation}, driving custom-designed \textbf{servo-hydraulic actuators}.
\item[$\bullet$] Developed a \textbf{force-feedback Zero-G controller} using \textbf{F/T sensing} and \textbf{spring-damper admittance} at \textbf{1 kHz}, \textbf{7 DoF}.
\item[$\bullet$] Generated \textbf{2K} frame synthetic dataset in \textbf{IsaacLab} with randomized solar panels, lights, pose, and terrain, on \textbf{DGX Spark}.
\item[$\bullet$] Trained \textbf{RF-DETR-Seg Nano} for \textbf{solar panel instance segmentation} on \textbf{D555}: \textbf{0.512 mAP}, \textbf{0.939 IoU}, \textbf{p95:20.3 ms}.
\item[$\bullet$] Exported trained model via \textbf{ONNX} and \textbf{TensorRT} for low-latency inference on \textbf{Jetson edge compute}.
\item[$\bullet$] Built \textbf{RANSAC}+\textbf{PCA} pipeline on masked \textbf{depth data} to estimate solar panel \textbf{6-DoF pose}, \textbf{dimensions}, \textbf{mass}.
\item[$\bullet$] Building a \textbf{digital twin} of the \textbf{robot and workcell scene} in \textbf{IsaacLab} for higher-fidelity simulation.
\resumeItemListEnd
...
\small{\begin{tabular*}{\textwidth}[t]{p{1\textwidth}}
    \hspace{-3.1mm}{\textbf{ Control Systems:} PID, LQR, PIV, Numerical Optimization (ProxQP, OSQP), Model Predictive Control} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Robotics \& Simulation:} Pinocchio, OMPL, nvblox, CoppeliaSim, MuJoCo, ROS2, FastDDS, IsaacLab} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Perception \& CV Models:} YOLO26, RF-DETR, YOLACT-Edge, Instance Segmentation, Intel RealSense D555} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Embedded Hardware:} Nvidia Jetson, Raspberry Pi, Arduino, ESP32, STM32 (Moteus X1)} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Programming \& Libraries:} C++, Python, C, Embedded C, CUDA, PyTorch, OpenCV, SciPy, MATLAB/Simulink} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Communication Protocols:} CAN, CAN-FD, UART, USB, I2C, SPI, BLE, WiFi, MQTT, Dynamixel Protocol 2.0, RTPS} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Dev \& Test Tools:} Git, Subversion, Docker, CMake, Linux Bash, Jira, LabVIEW, LPKF CircuitPro, Overleaf} \vspace{1mm}\\
    \hspace{-3.1mm}{\textbf{ Certifications:} \href{https://www.credly.com/badges/258a5ac5-9551-4cef-a26b-0a80f7c555cd/public_url}{PMI Certified Associate in Project Management (CAPM)}} \vspace{1mm}
\end{tabular*}}
