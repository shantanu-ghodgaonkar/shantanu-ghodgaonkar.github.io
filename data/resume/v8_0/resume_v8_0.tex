...
\item[$\bullet$] Built a \textbf{modular, platform-independent C++ framework} for control and planning across \textbf{ROS2, FastDDS}.
\item[$\bullet$] Designed a \textbf{GPU-accelerated (CUDA), collision-aware inverse kinematics (IK)} solver converging in \textbf{70–80 ms}.
...
\item[$\bullet$] Built a \textbf{PCD-to-ESDF} module enabling \textbf{environment-aware motion planning from 3D sensor data}.
\item[$\bullet$] Implemented \textbf{RRT-Connect} with \textbf{GPU-accelerated environmental collision checks} for motion planning.
...
\item[$\bullet$] Developing \textbf{eye-in-hand visual servoing} (\textbf{D555}, \textbf{instance segmentation}, \textbf{90\% mAP}) feeding pose to \textbf{IK/MPC pipeline}.
\item[$\bullet$] Benchmarking \textbf{YOLO26, RF-DETR, and YOLACT-Edge} for \textbf{instance segmentation} to pick the top performer.
...
\item[$\bullet$] Validating control and planning algorithms in \textbf{CoppeliaSim, MuJoCo and IsaacLab}, targeting \textbf{Jetson Thor} deployment.
\resumeItemListEnd
...
\small{\begin{tabular*}{\textwidth}[t]{p{1\textwidth}}
    ...
    \hspace{-3.1mm}{\textbf{ Communication Protocols:} CAN, CAN-FD, UART, USB, I2C, SPI, BLE, WiFi, MQTT, Dynamixel Protocol 2.0} \vspace{1mm}\\
    ...
    \hspace{-3.1mm}{\textbf{ Certifications:} PMI Certified Associate in Project Management (CAPM) -- Exam Scheduled: Aug 8, 2026} \vspace{1mm}
\end{tabular*}}
